...
export const useFetchPostingan = () => {
    ...
    const { kelas_id , mata_pelajaran_id, guru_id } = useParams(); 
    // const mata_pelajaran_id = parseInt(id) 
    return useQuery({
        queryKey: ['postingan', kelas_id , mata_pelajaran_id, guru_id],
        queryFn: async () => { 
            const token = Cookies.get('token') 
            const postinganResponse = await axiosInstance.get(`/postingan/kelas/${kelas_id}/mata_pelajaran/${mata_pelajaran_id}/guru/${guru_id}`, {
                headers: {
        ...
        enabled: !!kelas_id && !!mata_pelajaran_id && !!guru_id, // Hanya jalankan query jika tugas_id tersedia
