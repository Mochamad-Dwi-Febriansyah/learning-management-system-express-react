...
import { useDeleteSiswa } from "../../../features/Admin/Siswa/useDeleteSiswa.js";
import { useCreateSiswa } from "../../../features/Admin/Siswa/useCreateSiswa.js";
import { useEditSiswa } from "../../../features/Admin/Siswa/useEditSiswa.js";
...
const Siswa = () => {
    ...
    const { mutate: createSiswa, isLoading: createSiswaIsLoading } = useCreateSiswa({
        onError: (error) => { 
    ...
    const { mutate: deleteSiswa } = useDeleteSiswa({
        onSuccess: () => {
            refetchSiswa()
          }
    })
    const { mutate: editSiswa,isLoading: editSiswaIsLoading } = useEditSiswa({
        onError: (error) => { 
            if (error.response && error.response.data.errors) {
                const formattedErrors = error.response.data.errors.reduce((acc, err) => {
                    acc[err.path] = err.msg
    ...
    const { data, error, isLoading: siswaIsLoading,  refetch: refetchSiswa } = useFetchSiswa(); 
    ...
    const onEditClick = (siswa) => {
        console.log(siswa)
        formik.setFieldValue("id", siswa.id)
        formik.setFieldValue("nama", siswa.nama) 
        formik.setFieldValue("nama_akhir", siswa.nama_akhir) 
        formik.setFieldValue("nis", siswa.nis) 
        formik.setFieldValue("nomor_hp", siswa.nomor_hp) 
        formik.setFieldValue("tanggal_lahir", siswa.tanggal_lahir)  
        formik.setFieldValue("agama", siswa.agama) 
        formik.setFieldValue("alamat", siswa.alamat) 
        formik.setFieldValue("jenis_kelamin", siswa.jenis_kelamin) 
        formik.setFieldValue("user_type", siswa.user_type) 
        formik.setFieldValue("is_active", siswa.is_active ? "true" : "false");
        formik.setFieldValue("is_delete", siswa.is_delete ? "true" : "false");
        formik.setFieldValue("email", siswa.email) 
        formik.setFieldValue("password", siswa.password) 
    ...
    const renderSiswa = () => {
        return data?.data.data.map((siswa) => {
            return (
                <tr key={siswa.id}>
                    <td><p className="text-xs text-dark mb-0">{siswa.nama} {siswa.nama_akhir}</p></td>
                    <td><p className="text-xs text-dark mb-0">{siswa.nis}</p></td>
                    <td><p className="text-xs text-dark mb-0">{siswa.nomor_hp}</p></td>
                    <td><p className="text-xs text-dark mb-0">{siswa.tanggal_lahir}</p></td>
                    <td><p className="text-xs text-dark mb-0">{siswa.email}</p></td>
                    <td><p className="text-xs text-dark mb-0">{siswa.alamat}</p></td>
                    <td className="align-middle text-center">
                        <button className="btn btn-link text-dark px-2 mb-0" onClick={() => onEditClick(siswa)}><i className="fas fa-pencil-alt text-dark" aria-hidden="true"></i></button>
                        <button className="btn btn-link text-danger text-gradient px-2 mb-0" onClick={() => confirmationDelete(siswa.id) }><i className="far fa-trash-alt " aria-hidden="true"></i></button>
                    </td>
                </tr>
            )
        })
                            ...
                            <table className="table align-items-center m-3">
                                ...
                                <tbody>
                                    ...
                                    <tr className="text-center">
                                        <td> 
                                        {siswaIsLoading ? <div className="spinner-border text-primary" role="status">
                                        ...
                                        <form onSubmit={formik.handleSubmit}>
                                            ...
                                            <div className="modal-footer">
                                                ...
                                                <button type="submit" className="btn badge badge-sm bg-gradient-primary m-1" data-bs-dismiss="modal">
                                                    {createSiswaIsLoading || editSiswaIsLoading ? <div className="spinner-border text-primary" role="status">
                                                        {/* <span className="visually-hidden">Loading...</span> */}
